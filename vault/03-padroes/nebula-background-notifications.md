...
>
> **Status split (2026-07-20):** the **notifications + permission-status** dimension **SHIPPED** as wave N15a / Nebula 0.12.0 → see [[nebula-notifications]] (it establishes the `#if !os(<platform>)` gating precedent, deviating from this note's original `@available(tvOS 26, unavailable)` / `if #available` recommendation — both empirically invalid for an already-unavailable SDK symbol). The **background-tasks** dimension **SHIPPED** as wave N15b / Nebula 0.13.0 → see [[nebula-background-tasks]] (it corrects this note's `#if canImport(BackgroundTasks)` whole-file-gate recommendation — **empirically WRONG**: the framework is physically present on macOS/watchOS so `canImport` is `true` there and gates nothing; the correct gate is the **type-level `@available(macOS, unavailable) @available(watchOS, unavailable)` declaration** on the Nebula façade, the `NebulaLogStoreExporter` precedent; and the non-`Sendable` `BGTask` requires a `@unchecked Sendable` reference-type box `NebulaBGTaskBox`, the `NebulaMemoryLogHandler` precedent — the plan's `Mutex<[String, BGTask]>`-no-`@unchecked` design hit a Swift 6 region-isolation wall). N15 split fully shipped.
