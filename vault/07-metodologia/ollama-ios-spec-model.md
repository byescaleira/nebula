...
3. **Publicar `byescaleira/ios-spec` sobre base pequena (`llama3.2` 3B, ~2 GB).** Cabe, push é finíssimo, `ollama pull` funciona em qualquer máquina. **Tradeoff brutal:** a inteligência-base é llama3.2 3B, **não** GLM 5.2 — o comportamento "especialista" vem só do SYSTEM, capacidade bruta é muito menor. É o que os comandos sugeridos pelo Ollama (`FROM llama3.2`) realmente constroem.
