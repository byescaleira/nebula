...
> Nebula 0.19.0 — aligned to OS 26 (Liquid Glass). **External navigation entries conforming to the router** (Wave N21): every way the app can be asked to navigate from the outside — **deep links** (`myapp://`), **universal links** (`https://`), **Spotlight/Handoff/Siri** `NSUserActivity`, **Home-screen shortcuts**, **notification taps**, and in-app "go here" — is funneled through one normalized ``NebulaLink`` value → an app-provided ``NebulaLinkParser`` port → a ``NebulaLinkRouter`` that drives the existing ``NebulaPresentationRouter``. **Additive, non-breaking** — new public types + one additive **default extension method** on `NebulaPresentationRouter` (`apply(_:)`); no protocol requirement changes, no `NebulaRoute`/`NebulaRouter`/`NebulaPresentation`/`NebulaSpyRouter`/Meridian `Router`/`MeridianNavigationStack` signature changes. Nebula stays Foundation-only (zero `import SwiftUI`/`UIKit`); the SwiftUI entry-point adapters live in the Meridian sibling. `dependencies: []` pristine. Supersedes the ad-hoc Wave III `URL → [Route]` function in the example app.

### Added — Wave N21 (external navigation entries conforming to the router)

- **Nebula (Foundation-only)** — `NebulaLink` (a `Sendable`/`Equatable`/`Hashable` struct normalizing *any* external navigation event — a `Source` enum `.urlScheme`/`.universalLink`/`.userActivity`/`.shortcut`/`.notification`/`.programmatic` + `url`/`identifier`/`title`/`payload`; `init(url:)` infers `.universalLink` for `http`/`https`, else `.urlScheme`, nil-scheme-safe; the non-Sendable Apple classes that originate these events are never stored — Sendable bits are extracted at the adapter boundary) + `NebulaLinkDestination<Route>` (the 5 cases `.unhandled`/`.present`/`.pushStack`/`.pushStackAndPresent`/`.dismiss`; `.unhandled` avoids an `Optional.none` clash) + `NebulaLinkParser<Route>` (the app-provided port, `Sendable`, primary associated type `Route` — sync/pure `resolve(_:)`; async deferred as a default extension) + `NebulaCompositeLinkParser<Route>` (first-non-`.unhandled`-wins over `[any NebulaLinkParser<Route>]`) + `NebulaLinkRouter<Router>` (the glue: `open(_:)` → `await router.apply(parser.resolve(link))`; holds the parser existentially; `Sendable` derived) + an additive **default extension method** `NebulaPresentationRouter.apply(_:)` (translates a destination into router intents — stack-rebuild cases `dismiss()` first to clear any stale modal, since `replaceStack` does not touch the modal slot; the harmless pop-when-no-modal is erased by the `replaceStack` that follows; the spy and the Meridian `Router` inherit it for free). Test doubles `NebulaStubLinkParser<Route>` + `NebulaSpyLinkParser<Route>` (`final class` + `let Mutex<[NebulaLink]>` derived `Sendable`), `public` in the product target.
- **Meridian (SwiftUI)** — `.meridianDeepLinks(_:)` (wires `.onOpenURL` — covers deep links + universal links; `URL` is `Sendable` so the `Task` capture is clean) + `.meridianUserActivity(_:_:)` (wires `.onContinueUserActivity` — Spotlight/Handoff/Siri; `NSUserActivity` is **not Sendable**, so the adapter builds the `NebulaLink` *inside* the perform closure before the `Task`, capturing only the `Sendable` link across the `@Sendable` `Task` boundary — no `#SendingClosureRisksDataRace`). Both modifiers verified available iOS 14/macOS 11/tvOS 14/watchOS 7 (visionOS via `*`) in the SwiftUI `.swiftinterface` → no `@available`/`#if os()` gates at Meridian's `.v26` floor. Shortcuts (`UIApplicationShortcutItem`, UIKit) and notifications (`UNNotificationResponse`) have no SwiftUI View modifier / pull UIKit → the app builds a `NebulaLink` in its delegate and calls `linkRouter.open(_:)` (documented recipe — Meridian stays UIKit-free). The `MeridianExampleApp` migrates the ad-hoc `URL → [Route]` to an `AppLinkParser: NebulaLinkParser` + `NebulaLinkRouter` + `.meridianDeepLinks`/`.meridianUserActivity`, with an in-app `@Environment(\.openURL)` button to exercise the path.

### Notes

- **One navigation seam for external + in-app**: every external entry is normalized to a `NebulaLink`, resolved by a parser to a `NebulaLinkDestination`, and `apply`-ed to the same `NebulaPresentationRouter` the viewmodels use — deep links, universal links, Spotlight, shortcuts, notifications, and in-app buttons all conform to the router.
- **Additive `apply(_:)` default extension, not a requirement**: `NebulaPresentationRouter` gains a default extension method (not a protocol requirement) → `NebulaSpyRouter`/Meridian `Router`/any app router inherit it; zero conformance burden, no breaking change. The destination→intents translation lives once.
- **Atomicity**: `.pushStack`/`.pushStackAndPresent` `dismiss()` first to clear a stale modal (the harmless pop-when-no-modal is erased by `replaceStack`); `.present`/`.dismiss` are additive over the current state.
- **`NSUserActivity` Sendable boundary**: the Meridian adapter extracts the activity's `activityType`/`title`/`webpageURL`/`userInfo→[String:String]` and builds the `NebulaLink` before the `Task` — only the `Sendable` link crosses the `@Sendable` `Task` closure.
- **No breaking change**; additive minor within Nebula 26. Nebula `dependencies: []` pristine (Meridian lists only the local Nebula sibling). 1007 Nebula tests / 203 suites + 24 Meridian tests / 3 suites green (+22 Nebula / +4 Meridian over 0.18.0); zero concurrency warnings; release clean; all 5 platforms compile the new Nebula types ungated; Meridian modifiers ungated.

## [0.18.0] - 2026-07-22

> Nebula 0.18.0 — aligned to OS 26 (Liquid Glass). **Presentation navigation styles + container adapters** (Wave N20): the per-route presentation-style + modern-container layer over the Wave I/II presentation architecture. A route now declares **how it is presented** (`.push`/`.sheet`/`.fullScreenCover`) and the modern container trio (`NavigationStack` + `NavigationSplitView` + `TabView`) is bound to the router, pickable per screen area. **Additive, non-breaking** — new public types + an additive default on `NebulaRoute`; existing `NebulaRouter`/`MeridianNavigationStack` conformers/callers keep working. Nebula stays Foundation-only (zero `import SwiftUI`/`UIKit`); all SwiftUI lives in the Meridian sibling. `dependencies: []` pristine.

### Added — Wave N20 (presentation navigation styles + container adapters)

- **Nebula (Foundation-only)** — `NebulaPresentationStyle` (`Sendable`/`Equatable`/`Hashable`/`Codable` enum `.push`/`.sheet`/`.fullScreenCover`, `isModal`) + an additive `NebulaRoute.presentationStyle` requirement with a `.push` default extension (existing conformers auto-conform — non-breaking) + `NebulaPresentation<Route>` (the navigation-as-data model owning the typed `[Route]` push path **and** a single modal slot; push-path ops delegate to the `NebulaNavigationStack` statics, present/dismiss live in `static` `inout`-component helpers — single source of truth, no duplicated stack logic) + `NebulaPresentationRouter<Route>` (a sub-protocol of `NebulaRouter` adding async `present(_:)`/`present(_:as:)`/`dismiss()`; existing `NebulaRouter` conformers untouched) + extended `NebulaSpyRouter` (new `Intent` cases `.present`/`.presentAs`/`.dismiss`; conforms to `NebulaPresentationRouter`).
- **Meridian (SwiftUI)** — `Router` gains observable `presented`/`presentedStyle` and conforms to `NebulaPresentationRouter` (delegates to `NebulaPresentation` statics — single source of truth) + `MeridianNavigationStack` gains `.sheet(isPresented:)`/`.fullScreenCover(isPresented:)` gated bindings driven by the single modal slot (NOT `.sheet(item:)` — `Route` need not be `Identifiable`; `fullScreenCover` gated `#if !os(macOS)` with a `.sheet` fallback on macOS since the API is unavailable there) + new `MeridianNavigationSplitView` (sidebar + a `MeridianNavigationStack` detail — `NavigationSplitView` subsumes the deprecated `NavigationView`) + new `MeridianTabView` (`TabView` with `Tab(value:)`, `Tab: CaseIterable & Hashable & Sendable`, one `Router` per tab — never share a path across tabs) + an extended `MeridianExampleApp` (a `TabView` root with a split-view items tab + a settings stack tab, exercising `.share` as a sheet and `.login` as a cover).

### Notes

- **Per-route style is the dispatch key**: `present(_:)` dispatches by the route's declared `presentationStyle` (a `.push` route pushes; a `.sheet`/`.fullScreenCover` route fills the single modal slot); `present(_:as:)` overrides the declared style at the call site (the "present this `.push` route as a sheet *here*" escape hatch). `dismiss()` clears an active modal, else pops one — never both.
- **Single modal slot**: one sheet/cover at a time (matches SwiftUI). `presented` + `presentedStyle` pick the container; the two gated `isPresented` bindings are mutually exclusive.
- **Modern trio only** (no deprecated `NavigationView`): `NavigationStack` (push) + `NavigationSplitView` (split) + `TabView` (`Tab(value:)`, one `Router` per tab).
- **Additivity**: `NebulaPresentationRouter` is a sub-protocol and `NebulaRoute.presentationStyle` has a `.push` default → existing conformers/callers unbroken. `NebulaRouter`/`NebulaNavigationStack`/`MeridianNavigationStack(router:root:destination:)` existing API preserved.
- **No breaking change**; additive minor within Nebula 26. Nebula `dependencies: []` pristine (Meridian lists only the local Nebula sibling). 985 Nebula tests / 198 suites + 20 Meridian tests / 3 suites green; zero concurrency warnings; release clean; all 5 platforms compile the new Nebula types ungated; DocC builds.

## [0.17.0] - 2026-07-21

> Nebula 0.17.0 — aligned to OS 26 (Liquid Glass). **CloudKit-backed observability suite** (Wave N19): a Metrics + Analytics + CloudKit-sync + performance-sink surface shipped as ports + CloudKit adapters in Nebula core, opt-in via configuration. **Corrects the prior "Defer (sibling)" CloudKit verdict** — verified against the authoritative `CloudKit.swiftmodule` `.swiftinterface` (Xcode 27 Beta 3 / Swift 6.4): `CKContainer`/`CKDatabase`/`CKSyncEngine` are clean `Sendable` on all 5 platforms below the `.v26` floor → no `@available` gate, no `#if os()`; the `@unchecked` CK value types are Apple classes (not Nebula value types), handled by the `final class @unchecked` box + copy-to-Sendable at the adapter boundary. `dependencies: []` pristine (CloudKit is a system framework).

### Added — Wave N19 (CloudKit-backed observability)

- **Metrics** (`Sources/Nebula/Metrics/`): `NebulaMetrics` port (`record(_:)` + counter/histogram/gauge/timing default extensions) + `NebulaMetricEvent`/`NebulaMetricValue`/`NebulaMetricKind` + `NebulaEventBuffer<Event: Sendable & Equatable>` (final-class `Mutex<[Event]>` batcher) + `NebulaMetricsConfiguration` (Sendable-NOT-Equatable, `@Sendable` handler) + `NebulaMetricsConfig` (`Mutex` accessor) + `NebulaLocalMetrics`.
- **Analytics** (`Sources/Nebula/Analytics/`): `NebulaAnalytics` port (`track(_:)` + track/screen/identify default extensions) + `NebulaAnalyticsEvent` + `NebulaAnalyticsConfiguration`/`NebulaAnalyticsConfig`/`NebulaLocalAnalytics` (reuses `NebulaEventBuffer`).
- **Performance sink** (`Sources/Nebula/Metrics/NebulaPerformanceSink.swift`): routes each `NebulaMeasureResult` from the Measure subsystem into `NebulaMetricsConfiguration` as a `.timing` event (no new timing API).
- **CloudKit glue** (`Sources/Nebula/Architecture/CloudKit/`): `NebulaCloudKitEnvironment` (Nebula-owned `Sendable, Equatable` enum — `CKContainer.Environment` does NOT exist as a Swift type in the Xcode 27 Beta 3 SDK) + `NebulaCloudKitConfiguration` (`Sendable, Equatable`; no handler) + `NebulaCloudKitConfig` (`Mutex` accessor) + `NebulaCloudKitSync` (port: `sendChanges`/`fetchChanges` + `sync()` default ext) + `NebulaCloudKitSyncEngine` (`final class` wrapping `CKSyncEngine`; container/database constructed in `init` and discarded; `@Sendable` delegate → `Sendable` via SE-0302; no `@unchecked` authored).
- **CloudKit-backed preferences (N19d)**: `NebulaCloudKitPreferences` (conforms `NebulaPreferences`) — local synchronous cache + injectable async sink that flushes each `NebulaCloudKitKVChange` to CloudKit (default sink maps a key/value to a `CKRecord`, save/delete stateless per call — the `NebulaKeychain` precedent, no `CKDatabase` stored, `Sendable` derived with no `@unchecked`) + `ensureZone()`/`refresh()` extras.
- **CloudKit-backed feature flags (N19d)**: `NebulaCloudKitFeatureFlags` (conforms `NebulaRemoteFeatureFlags`) — **read-only**: `value(forKey:)` serves a local `Mutex<[String: NebulaFlagValue]>` cache; `refresh()` pulls `NebulaFlag` records and replaces the cache, leaving it unchanged on failure (the port contract); stateless-per-call CloudKit boundary (no `CKDatabase` stored, `Sendable` derived with no `@unchecked`); injectable `fetch` (default `defaultFetch` is compile-verified — runtime needs iCloud); `encode(_:into:)`/`decode(from:)` public record codec (a `kind` tag + per-kind field — pure, unit-testable with an in-memory `CKRecord`, no iCloud).
- **DocC**: `ArchitectureCloudKitObservability.md` article (indexed in `Architecture.md`).
- **Tests**: `MetricsTests.swift` + `AnalyticsTests.swift` + `CloudKitObservabilityTests.swift` + `CloudKitFeatureFlagsTests.swift`.
- **Vault**: ADR `02-decisoes/adr-cloudkit-observability-suite.md` + design note `03-padroes/nebula-cloudkit-observability.md`; `Home.md` CloudKit shipped link; hub `nebula-app-readiness-research.md` CloudKit row corrected (Defer → Façade in Nebula core, ✅ shipped 0.17.0).

### Notes

- **CloudKit admissibility (load-bearing)**: `CKContainer`/`CKDatabase`/`CKSyncEngine` are clean `Sendable` (no `@unchecked`) on all 5 platforms, all below the `.v26` floor → no `@available` gate, no `#if os()` on the core sync path. The `@unchecked` CK value types (`CKRecord`/`CKQuery`/`CKShare`/…) are Apple classes, not Nebula value types — the binding's "no `@unchecked` on Nebula-defined value types" rule does not forbid them; handled by the `final class @unchecked` box (the `NebulaMemoryLogHandler`/`NebulaBGTaskBox` precedent) + copy-to-Sendable at the adapter boundary (`NebulaLogStoreExporter`). `CLAUDE.md` allowed-frameworks list gains `CloudKit`.
- **Stateless-per-call CloudKit boundary**: no `CKDatabase` is stored on any Nebula conformer (the `NebulaKeychain` precedent) — `CKContainer`/`CKDatabase` are resolved fresh per call, so their preconcurrency Sendability never reaches a Nebula `Sendable` derivation; `Sendable` is derived with **no `@unchecked`** on every Nebula-defined value/reference type.
- **Opt-in via configuration**: every config defaults to a capture-free no-op handler and `isEnabled = false` for the CloudKit path — a foundation does not implicitly hit the network. The app opts in at the composition root (`Nebula*Config.set(_:)` or explicit-param DI), supplying `containerIdentifier` + entitlements. Nebula ships compile-verified adapters; the live CloudKit container is app-owned (the `MeridianExample` precedent — a Nebula sample can't ship a working container).
- **Testability**: the Metrics/Analytics event-buffer + config + local façade are unit-tested; the CloudKit preferences sync cache + the recording-sink path are unit-tested; the CloudKit feature-flags cache/refresh contract is unit-tested via an injectable fake `fetch`; the `encode`/`decode` record codec is unit-tested with an in-memory `CKRecord` (no iCloud). The default CloudKit fetch/sink are compile-verified (runtime needs an iCloud entitlement + account — app-owned).
- **No breaking change**; additive minor within Nebula 26. `dependencies: []` pristine (CloudKit is a system framework, not an SPM dep). 963 tests / 194 suites green; zero concurrency warnings; release clean; all 5 platforms compile the CloudKit path ungated; DocC `BUILD DOCUMENTATION SUCCEEDED`.
